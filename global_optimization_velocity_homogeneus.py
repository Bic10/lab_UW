...
for chosen_uw_file, infile_path in enumerate(infile_path_list_uw):
    ...
    for idx_waveform, (thickness_gouge_1, thickness_gouge_2, normal_stress, shear_stress) in enumerate(zip(thickness_gouge_1_list[::downsampling], thickness_gouge_2_list[::downsampling], normal_stress_list[::downsampling], shear_stress_list[::downsampling])):
        ...
        if previous_min_velocity is None:
            ...
            if max_signal/max_noise < minimum_SNR:
                print(f"Signal to Noise ratio for waveform {idx} = {max_signal/max_noise}. Skipped computation")
                continue

            # First waveform, use initial velocity range, corrected for an empirical depdendence from normal stress

            correction_normal_stress = normal_stress**0.25
            ...
            c_range = 200 * (1e2 / 1e6)  # [cm/μs], adjust as needed
            cmin_waveform = previous_min_velocity - c_range
            cmax_waveform = previous_min_velocity + c_range
            c_step_waveform = c_step   # Use a smaller step size for better resolution

        # DEFINE EVALUATION INTERVAL FOR L2 NORM OF THE RESIDUALS
        max_travel_time = fixed_travel_time + thickness_gouge_1 / cmin_waveform + thickness_gouge_2 / cmin_waveform + 0.5*(2*h_groove_side + 2*h_groove_central )/ (steel_velocity + cmin_waveform)
        min_travel_time = fixed_travel_time + thickness_gouge_1 / cmax_waveform + thickness_gouge_2 / cmax_waveform + 0.5*(2*h_groove_side + 2*h_groove_central )/ (steel_velocity + cmax_waveform)
        ...
        print(f"Waveform {idx}: Minimum misfit at gouge_velocity = {previous_min_velocity:.4f} cm/μs")
